...
@section ('content')
    <div id="thread" class="v-thread">
                ...
                @if (! $thread->trashed())
                    @can ('reply', $thread)
                        <div class="btn-group" role="group">
                            <a href="{{ Forum::route('post.create', $thread) }}" class="btn btn-warning">
        ...
        @if ((count($posts) > 1 || $posts->currentPage() > 1) && (Gate::allows('deletePosts', $thread) || Gate::allows('restorePosts', $thread)) && count($selectablePosts) > 0)
            <div class="text-end pb-1">
                <div class="form-check">
                    <label for="selectAllPosts" class="text-white">
        ...
        @if (! $thread->trashed())
            @can ('reply', $thread)
                <h3 class="text-white mt-5">{{ trans('forum::general.quick_reply') }}</h3>
                <div id="quick-reply">
                    <form method="POST" action="{{ Forum::route('post.store', $thread) }}">
                        @csrf

                        <div class="mb-3">
                            <textarea name="content" class="form-control bg-transparent text-white border border-warning rounded">{{ old('content') }}</textarea>
                        </div>
