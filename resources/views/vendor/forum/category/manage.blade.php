...
@section ('content')
    <div class="d-flex flex-row justify-content-between mb-2">
        <h2 class="flex-grow-1 text-white">{{ trans('forum::general.manage') }}</h2>

        @can ('createCategories')
            <button type="button" class="btn btn-warning" data-open-modal="create-category">
    ...
    <script type="text/x-template" id="draggable-category-list-template">
        <draggable tag="ul" class="list-group" :list="categories" group="categories" :invertSwap="true" :emptyInsertThreshold="14">
            <li class="list-group-item bg-transparent border border-warning my-3" v-for="category in categories" :data-id="category.id" :key="category.id">
                ...
                <div class="text-white">@{{ category.description }}</div>
