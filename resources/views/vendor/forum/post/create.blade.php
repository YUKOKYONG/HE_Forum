...
@section ('content')
    <div id="create-post">
        <h2 class="text-white">{{ trans('forum::general.new_reply') }} ({{ $thread->title }})</h2>
        ...
        <form method="POST" action="{{ Forum::route('post.store', $thread) }}">
            ...
            <div class="mb-3">
                <textarea name="content" class="form-control bg-transparent border border-warning rounded text-white">{{ old('content') }}</textarea>
            </div>
