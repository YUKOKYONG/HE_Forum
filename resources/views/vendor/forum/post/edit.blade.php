...
@section ('content')
    <div id="edit-post">
        <h2 class="flex-grow-1 text-white">{{ trans('forum::posts.edit') }} ({{ $thread->title }})</h2>
        ...
        <form method="POST" action="{{ Forum::route('post.update', $post) }}">
            ...
            <div class="mb-3">
                <textarea name="content" class="form-control bg-transparent border border-warning rounded text-white">{{ old('content') !== null ? old('content') : $post->content }}</textarea>
            </div>
